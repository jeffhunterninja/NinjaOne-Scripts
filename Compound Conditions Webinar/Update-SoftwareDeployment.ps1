    ...
    if ($Option1 -ieq "ALL") {
        ...
        foreach ($organization in $organizations) {
            if ($null -ne $organization) {
                ...
                $requestBody = @{
                    softwareToInstallOrganization = $softwareList
            ...
            foreach ($organization in $organizations) {            
                if ($null -ne $organization) {
                    ...
                    try {
                        ...
                        $existingValue = $customFields.softwareToInstallOrganization -as [string]
                    } catch {
                    ...
                    $requestBody = @{
                        softwareToInstallOrganization = $updatedValue
                    } | ConvertTo-Json -Depth 10
        ...
        if ($Option8) {
            ...
            foreach ($DeviceName in $DeviceNames) {
                ...
                if ($null -ne $device) {
                    ...
                    try {
                        $customFields = Invoke-RestMethod -Uri $customFieldsUrl -Method GET -Headers $headers
                        $existingValue = $customFields.softwareToInstallDevice.text -as [string]
                    } catch {
                    ...
                    $requestBody = @{
                        softwareToInstallDevice = $updatedValue
                    } | ConvertTo-Json -Depth 10
        ...
        if ($Option10) {
            ...
            foreach ($DeviceName in $DeviceNames) {
                ...
                if ($null -ne $device) {
                    ...
                    try {
                        $customFields = Invoke-RestMethod -Uri $customFieldsUrl -Method GET -Headers $headers
                        $existingValue = $customFields.softwareToInstallDevice -as [string]
                    } catch {
                        Write-Error "Failed to retrieve custom fields for $($DeviceName): $_"
                        continue
                    }
