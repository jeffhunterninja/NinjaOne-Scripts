...
.DESCRIPTION
    ...
    from NinjaOne custom fields, set by Set-CustomFieldPatchingSchedule.ps1. Supports Daily (every day at same time),
    ...
    - patchingRecurrence  : Optional. Daily | Weekly | Monthly. When not set: Daily if patchingDay is empty; Weekly if patchingDay is set.
    ...
    - disablePatching   : Optional. When true, script exits 1 immediately and patching is skipped (control switch to cancel patch job).
...
try { $v = Get-NinjaProperty patchingRecurrence -type dropdown; if ($null -ne $v -and -not [string]::IsNullOrWhiteSpace([string]$v)) { $PatchingRecurrence = $v } } catch { }
try { $v = Get-NinjaProperty patchingOccurrence -type dropdown;  if ($null -ne $v -and -not [string]::IsNullOrWhiteSpace([string]$v)) { $PatchingOccurrence  = $v } } catch { }
$PatchingDay   = Get-NinjaProperty patchingDay
...
# Normalize recurrence: Daily | Weekly | Monthly. When not set: Daily if patchingDay is empty, Weekly if patchingDay is set.
if ([string]::IsNullOrWhiteSpace($PatchingRecurrence)) {
    if ([string]::IsNullOrWhiteSpace($PatchingDay)) { $PatchingRecurrence = 'Daily' }
...
if ([string]::IsNullOrWhiteSpace($PatchingStart)) {
    Write-Error "PatchingStart is required. Set the patchingStart custom field in NinjaOne (via Set-CustomFieldPatchingSchedule.ps1 or the NinjaOne UI)."
    exit 2
}
...
# Format A: HH:mm (interpreted as device local time; no UTC conversion)
...
# Format C: Unix milliseconds (e.g. from Set-CustomFieldPatchingSchedule.ps1 import for Weekly)
