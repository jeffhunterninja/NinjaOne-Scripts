...
**v1** - Initial release on 2025-03-27

**v2** — Code cleanup, bugfixes on 2026-02-16

**v3** - Added support for Monthly, Day of Week as a mode and option for Window Recurrence. Changed exit code structure to match other scripts.
...
| `2` | Error (validation/parse error, missing required parameters) |

Configure the compound condition so that **exit code 1** is treated as condition met (trigger).

When **InvertExitCode** is enabled, exit codes 0 and 1 are swapped (2 is unchanged); configure the compound condition so that **exit code 0** is treated as condition met. Realistically this only makes sense to use with Window mode.
...
| `Monthly Occurrence`         | Monthly, Day of Week mode      | DROPDOWN      | `1`, `2`, `3`, `4`, or `Last` (e.g. second Tuesday, last Saturday)    |
...
| `Window Monthly Occurrence`  | Monthly window recurrence      | DROPDOWN      | Required if `Window Recurrence` is `Monthly`. Values: `1`, `2`, `3`, `4`, or `Last` |
| `Time Window Minutes`        | Grace period (minutes)         | INTEGER   | Optional. Set to interval + 1 (e.g., 16 for 15-min interval). Default 5. |
| `Invert Exit Code`           | Invert success/not-met codes   | CHECKBOX  | Optional. When true, exit 0/1 are swapped; 2 unchanged. Env: `invertExitCode` (true, 1, yes). |
...
- **Examples**: Second Tuesday at 02:00 → Day Of Week `Tuesday`, Monthly Occurrence `2`; last Saturday at 23:00 → `Saturday`, `Last`
