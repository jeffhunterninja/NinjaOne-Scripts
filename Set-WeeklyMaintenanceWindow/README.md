...
1. **Set-WeeklyMaintenanceWindow.ps1** – Sets maintenance windows for organizations, locations, and devices in NinjaOne by importing values from a CSV via the NinjaOne API. Supports **Daily**, **Weekly**, and **Monthly** (nth weekday of month) recurrence. Run this locally on your workstation, or via an API server.
2. **Check-WeeklyMaintenanceWindow.ps1** – Checks whether the device's current local time is within or outside a recurring maintenance window (Daily, Weekly, or Monthly) for use in NinjaOne compound conditions or with a pre-script.
...
If `maintenanceRecurrence` is missing, **Weekly** is assumed for backward compatibility.
...
- **CSV columns**
  ...
  - `maintenanceStart`, `maintenanceEnd`: Start and end time as HH:mm (e.g. `02:00`, `22:00`). For Weekly, these are converted to Unix milliseconds on import; for Daily and Monthly they are stored as HH:mm.
...
Rename the example columns to match your NinjaOne custom field names.
...
- **Input formats**: HH:mm (e.g. `02:00`), seconds from midnight UTC (0–86400), or Unix milliseconds (as stored by Set-WeeklyMaintenanceWindow for Weekly).

### Input (Check script – environment variables from NinjaOne custom fields)

| Environment variable      | Required              | Description                                                                 |
|---------------------------|-----------------------|-----------------------------------------------------------------------------|
| `maintenanceRecurrence`   | No (default: Weekly)  | `Daily`, `Weekly`, or `Monthly`.                                           |
...
Maintenance windows are typically populated by **Set-WeeklyMaintenanceWindow.ps1**. That script reads a CSV with recurrence type and window columns and writes them to NinjaOne custom fields. For **Weekly** recurrence, `maintenanceStart` and `maintenanceEnd` are converted to Unix milliseconds on import; for **Daily** and **Monthly** they are stored as HH:mm. Check-WeeklyMaintenanceWindow.ps1 accepts HH:mm, seconds-from-midnight-UTC, and Unix ms for start/end, and uses `maintenanceRecurrence` to apply Daily, Weekly, or Monthly window logic.
