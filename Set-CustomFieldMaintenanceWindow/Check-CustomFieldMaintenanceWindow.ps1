...
.DESCRIPTION
    ...
    NinjaOne custom fields, set by Set-CustomFieldMaintenanceWindow.ps1. NinjaOne injects device/org/location custom
    ...
    - maintenanceRecurrence : Optional. Daily | Weekly | Monthly. When not set: Daily if maintenanceDay is empty; Weekly if maintenanceDay is set.
    ...
    - disablePatching       : Optional (patching control). When true, script exits 1 immediately so the patch job is cancelled.
...
# Disable Patching control switch: if checked, exit immediately so patch job is cancelled
$DisablePatching = $env:disablePatching -as [string]
try { $v = Get-NinjaProperty disablePatching; if ($null -ne $v -and -not [string]::IsNullOrWhiteSpace([string]$v)) { $DisablePatching = [string]$v } } catch { }
$DisablePatching = -not [string]::IsNullOrWhiteSpace($DisablePatching) -and ($DisablePatching.Trim() -match '^(?i)(true|1|yes)$')
if ($DisablePatching) {
    Write-Output "Patching is disabled for this device/org/location."
    exit 1
}
...
try { $v = Get-NinjaProperty maintenanceRecurrence -type dropdown; if ($null -ne $v -and -not [string]::IsNullOrWhiteSpace([string]$v)) { $MaintenanceRecurrence = $v } } catch { }
try { $v = Get-NinjaProperty maintenanceOccurrence -type dropdown;  if ($null -ne $v -and -not [string]::IsNullOrWhiteSpace([string]$v)) { $MaintenanceOccurrence  = $v } } catch { }
...
# Normalize recurrence: Daily | Weekly | Monthly. When not set: Daily if maintenanceDay is empty, Weekly if maintenanceDay is set.
if ([string]::IsNullOrWhiteSpace($MaintenanceRecurrence)) {
    if ([string]::IsNullOrWhiteSpace($MaintenanceDay)) { $MaintenanceRecurrence = 'Daily' }
...
if ([string]::IsNullOrWhiteSpace($MaintenanceStart)) {
    Write-Error "MaintenanceStart is required. Set the maintenanceStart custom field in NinjaOne (via Set-CustomFieldMaintenanceWindow.ps1 or the NinjaOne UI)."
    exit 2
}
if ([string]::IsNullOrWhiteSpace($MaintenanceEnd)) {
    Write-Error "MaintenanceEnd is required. Set the maintenanceEnd custom field in NinjaOne (via Set-CustomFieldMaintenanceWindow.ps1 or the NinjaOne UI)."
    exit 2
}
...
# Format C: Unix milliseconds (e.g. from Set-CustomFieldMaintenanceWindow.ps1 import)
elseif ($MaintenanceStart -match '^\d+$' -and $MaintenanceEnd -match '^\d+$') {
    try {
