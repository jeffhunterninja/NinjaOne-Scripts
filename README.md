...
- **Set-CustomFieldMaintenanceWindow** ([Set-CustomFieldMaintenanceWindow/](Set-CustomFieldMaintenanceWindow/)) — Imports weekly maintenance windows from CSV to NinjaOne custom fields. Paired with Check-CustomFieldMaintenanceWindow.ps1 for compound conditions or patch cycle pre-scripts, allows your to establish a window where activity can occur. See [Set-CustomFieldMaintenanceWindow/README.md](Set-CustomFieldMaintenanceWindow/README.md).

- **Set-CustomFieldPatchingSchedule** ([Set-CustomFieldPatchingSchedule/](Set-CustomFieldPatchingSchedule/)) — Imports weekly patching schedules from CSV to NinjaOne custom fields at the organization, location, and device levels. In this workflow, policy patching is set to a schedule of *none*. Use Check-CustomFieldPatchingSchedule.ps1 as a script result condition (standard or compound) that triggers when the desired patching window is reached by using a patching automation to run in response when the automation is triggered. Approvals are still tied to distinct policies. See [Set-CustomFieldPatchingSchedule/README.md](Set-CustomFieldPatchingSchedule/README.md).
