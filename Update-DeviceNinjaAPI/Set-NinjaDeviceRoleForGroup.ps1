...
<#
.SYNOPSIS
  Sets the device role for all devices in a NinjaOne group via the API.

.DESCRIPTION
  Authenticates to NinjaOne using client credentials, retrieves all device IDs
  in the specified group, and PATCHes each device to set the given roleId.
  Role IDs can be obtained from GET {baseUrl}/api/v2/roles.
...
.PARAMETER GroupId
  NinjaOne group ID. All devices in this group will have their role updated.

.PARAMETER RoleId
  Role ID to assign to each device (e.g. Windows Workstation). Get IDs from /api/v2/roles.
...
.EXAMPLE
  .\Set-NinjaDeviceRoleForGroup.ps1 -GroupId 222 -RoleId 201
...
foreach ($deviceId in $deviceIds) {
    ...
    if ($PSCmdlet.ShouldProcess($deviceId, "Assign role $RoleId")) {
        try {
            Write-Progress -Activity "Updating devices in group $GroupId" -Status "Assigning role to device $current of $total" -PercentComplete ([int](100 * $current / $total))
            Invoke-RestMethod -Method Patch -Uri $deviceUpdateUrl -Headers $headers -Body $json -ContentType 'application/json'
            Write-Host "Assigning device role to $deviceId"
            $updated++
        } catch {
        ...
        Write-Host "[WhatIf] Would assign role $RoleId to $deviceId"
    }
