...
<#
.SYNOPSIS
  Moves all devices in a NinjaOne group to a specified organization and location via the API.

.DESCRIPTION
  Authenticates to NinjaOne using client credentials, retrieves all device IDs
  in the specified group, and PATCHes each device to set the given organizationId
  and locationId.
...
.PARAMETER GroupId
  NinjaOne group ID. All devices in this group will be moved.

.PARAMETER OrganizationId
  Target organization ID (integer).

.PARAMETER LocationId
  Target location ID within the organization (integer).
...
.PARAMETER WhatIf
  Preview which devices would be moved without applying changes.
...
.EXAMPLE
  ...
  .\Move-NinjaDevicesToOrgAndLocation.ps1 -GroupId 222 -OrganizationId 8 -LocationId 18 -WhatIf
#>
...
if ($OrganizationId -le 0 -or $LocationId -le 0) {
    Write-Error "OrganizationId and LocationId must be positive integers."
    exit 2
}
...
foreach ($deviceId in $deviceIds) {
    ...
    if ($PSCmdlet.ShouldProcess($deviceId, "Move to org $OrganizationId, location $LocationId")) {
        try {
            Write-Progress -Activity "Updating devices in group $GroupId" -Status "Moving device $current of $total" -PercentComplete ([int](100 * $current / $total))
            Invoke-RestMethod -Method Patch -Uri $deviceUpdateUrl -Headers $headers -Body $json -ContentType 'application/json'
            Write-Host "Moving device $deviceId to org $OrganizationId, location $LocationId"
            $updated++
        } catch {
        ...
        Write-Host "[WhatIf] Would move $deviceId to org $OrganizationId, location $LocationId"
    }
